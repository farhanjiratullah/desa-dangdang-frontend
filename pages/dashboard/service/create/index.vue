<template>
    <div>
        ...
        <div class="flex flex-row items-center justify-between">
            <div class="my-5 text-2xl font-bold">Tambah Service</div>    
        </div>

        <form @submit.prevent="handlerSubmit" method="post">
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Service</label>
                ...
                <div>
                    ...
                    <input type="text" v-model="tagline" id="tagline" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Pembuatan website ..." required />
                </div>
                <div>
                ...
                <p v-if="serviceError" class="text-sm text-red-600">{{ serviceError }}</p>
            </div>
...
definePageMeta({
  layout: 'admin',
  title: 'Admin Area | Create Service Section',
  middleware: [auth]
})
...
const handlerSubmit = async() => {
    try {
        ...
        const requestData = {
            ...
            path_icon: respImage.data.url,
        }
        
        await createData('/service-sections/admin', requestData)
