<template>
    <div>
        <NuxtLink to="/dashboard/service" class="flex items-center text-gray-900 hover:text-blue-800">
        ...
        <div class="flex flex-row items-center justify-between">
            <div class="my-5 text-2xl font-bold">Edit Service</div>    
        </div>
        ...
        <form @submit.prevent="handlerSubmit" method="post">
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" v-model="form.tagline" id="tagline" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Pembuatan website ..." required />
                </div>
                <div>
                ...
                <p v-if="form.serviceError" class="text-sm text-red-600">{{ form.serviceError }}</p>
            </div>
...
definePageMeta({
  layout: 'admin',
  title: 'Admin Area | Edit Service Section',
  middleware: [auth]
})
...
const getServiceByID = async() => {
    ...
    try {
        const dataService = await fetchDataByID("/service-sections/admin", id)
        form.value.name = dataService.data.name
        form.value.tagline = dataService.data.tagline
        form.value.imageUrl = dataService.data.path_icon
    } catch (error) {
        console.error('Error', error)
        form.value.serviceError = error.message
    }
}
