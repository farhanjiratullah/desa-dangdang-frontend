<template>
    <div>
        <NuxtLink to="/dashboard/client" class="flex items-center text-gray-900 hover:text-blue-800">
        ...
        <div class="flex flex-row items-center justify-between">
            <div class="my-5 text-2xl font-bold">Edit Client</div>    
        </div>
        ...
        <form @submit.prevent="handlerSubmit" method="post">
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                ...
                <p v-if="form.clientError" class="text-sm text-red-600">{{ form.clientError }}</p>
            </div>
...
const getClientByID = async() => {
    ...
    try {
        const dataClient = await fetchDataByID("/client-sections/admin", id)
